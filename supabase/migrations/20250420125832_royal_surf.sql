...
BEGIN
  ...
  -- Get admin user id for fee collection
  ...
  -- If payment completed, update balances and create transactions
  IF p_status = 'completed' THEN
    -- Update user's wallet balance with full amount
    UPDATE users
    SET 
    ...
    INSERT INTO wallet_transactions (
      ...
      'platform',
      'completed',
      'Platform fee from PayPal payment'
    );

    -- Update admin's balance with platform fee
    UPDATE users
    SET 
      wallet_balance = wallet_balance + v_fee_amount,
      updated_at = NOW()
    WHERE id = v_admin_id;
