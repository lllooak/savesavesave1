/*
  # Fix withdrawal status change trigger

  1. Changes
    - Improve the withdrawal status change trigger to properly handle balance updates
    - Add proper transaction recording for withdrawals
    - Add notification for withdrawal status changes
...
BEGIN
  ...
  IF (NEW.status = 'completed' AND OLD.status != 'completed') THEN
    ...
    INSERT INTO notifications (
      ...
      v_creator_id,
      'בקשת המשיכה אושרה',
      'בקשת המשיכה שלך בסך ₪' || v_amount || ' אושרה והסכום נמשך מחשבונך',
      'payment',
      NEW.id,
      'withdrawal'
    );
  ...
  ELSIF (NEW.status = 'rejected' AND OLD.status != 'rejected') THEN
    -- Create notification for the creator about rejection
    INSERT INTO notifications (
      user_id,
      title,
      message,
      ...
      v_creator_id,
      'בקשת המשיכה נדחתה',
      'בקשת המשיכה שלך בסך ₪' || v_amount || ' נדחתה. אנא פנה לתמיכה לפרטים נוספים.',
      'payment',
      NEW.id,
      'withdrawal'
    );
