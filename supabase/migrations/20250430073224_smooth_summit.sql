/*
  # Fix Video Request Flow in Admin Panel

  1. Changes
    - Update process_request_payment function to only deduct from fan's wallet
    - Update process_request_refund function to handle refunds properly
    - Update complete_request_and_pay_creator function to handle payments to creators
    - Update request_status_change_trigger to handle status changes properly
...
BEGIN
  ...
  -- Handle status change from declined to approved
  IF (NEW.status = 'approved' AND OLD.status = 'declined') THEN
    -- Get request details
    SELECT 
      fan_id, 
      creator_id, 
    ...
    -- Deduct amount from fan's wallet again
    UPDATE users
    SET 
      wallet_balance = wallet_balance - v_amount,
      updated_at = NOW()
    ...
    BEGIN
      ...
      -- Create transaction record for fan's payment
      INSERT INTO wallet_transactions (
        user_id,
        type,
        amount,
        ...
        'wallet',
        'completed',
        'Video request payment (after decline)',
        NEW.id::text
      );
      ...
      INSERT INTO notifications (
        ...
        v_fan_id,
        'בקשת הוידאו אושרה',
        'בקשת הוידאו שלך אושרה לאחר שנדחתה קודם לכן',
        'request',
        NEW.id,
        'request'
      );
      ...
      INSERT INTO notifications (
        ...
        v_creator_id,
        'בקשת וידאו אושרה',
        'בקשת וידאו אושרה לאחר שנדחתה קודם לכן',
        'request',
        NEW.id,
        'request'
      );
  ...
  -- Handle status change to approved (from pending)
