...
-- Create function to create notification when a message is received
...
-- Create function to create notification when a request status changes
...
BEGIN
  ...
  ELSIF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND NEW.status = 'pending' AND OLD.status IS DISTINCT FROM 'pending') THEN
...
-- Create function to create notification when a payment is processed
